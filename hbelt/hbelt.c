...
extern const u32 crypto_hbelt_h5[256] __alias(H5);
extern const u32 crypto_hbelt_h13[256] __alias(H13);
extern const u32 crypto_hbelt_h21[256] __alias(H21);
extern const u32 crypto_hbelt_h29[256] __alias(H29);

EXPORT_SYMBOL(crypto_hbelt_h5);
EXPORT_SYMBOL(crypto_hbelt_h13);
EXPORT_SYMBOL(crypto_hbelt_h21);
EXPORT_SYMBOL(crypto_hbelt_h29);
...
/* 512 -> 128 */
static void hbelt_sigma1(uint32_t *r, uint32_t *t, const uint32_t *u12,
                         const uint32_t *u34) {
...
/* 512 -> 256 */
static void hbelt_sigma2(uint32_t *r, uint32_t *k, const uint32_t *u12,
                         const uint32_t *u34) {
 ...
 * context.
 */
int hbelt_update(struct shash_desc *desc, const u8 *data, unsigned int len) {
...
EXPORT_SYMBOL_GPL(hbelt_final);

int hbelt_init(struct shash_desc *desc) {
    struct crypto_hbelt_ctx *ctx = shash_desc_ctx(desc);
...
EXPORT_SYMBOL_GPL(hbelt_init);

int hbelt_export(struct shash_desc *desc, void *out) {
    struct crypto_hbelt_ctx *ctx = shash_desc_ctx(desc);

    memcpy(out, ctx, sizeof(*ctx));
    return 0;
}
EXPORT_SYMBOL_GPL(hbelt_export);

int hbelt_import(struct shash_desc *desc, const void *in) {
    struct crypto_hbelt_ctx *ctx = shash_desc_ctx(desc);

    memcpy(ctx, in, sizeof(*ctx));
    return 0;
}
EXPORT_SYMBOL_GPL(hbelt_import);

static struct shash_alg alg = {.digestsize = HASH_SIZE_BELT_256,
                               .init = hbelt_init,
                               .update = hbelt_update,
                               .final = hbelt_final,
                               .export = hbelt_export,
...
static int __init hbelt_mod_init(void) { return crypto_register_shash(&alg); }

static void __exit hbelt_mod_fini(void) { crypto_unregister_shash(&alg); }
