...
int belt_expand_key(struct crypto_belt_ctx *ctx, const u8 *in_key,
    ...
    if ((key_len != BELT_KEYSIZE_128 && key_len != BELT_KEYSIZE_192 &&
         key_len != BELT_KEYSIZE_256) || in_key == NULL)
        return 1;
 ...
 */
static int belt_set_key(struct crypto_tfm *tfm, const u8 *in_key,
                       unsigned int key_len)
{
 ...
 */
static void decrypt_block(const uint32_t *key,
                          const unsigned char in_blk[], unsigned char out_blk[])
{
    register uint32_t a, b, c, d, e;
    ...
    dec_round(a,b,c,d,e,key[1],key[2],key[3],key[4],key[5],key[6],key[7],7);
    dec_round(c,a,d,b,e,key[2],key[3],key[4],key[5],key[6],key[7],key[0],6);
    dec_round(d,c,b,a,e,key[3],key[4],key[5],key[6],key[7],key[0],key[1],5);
    dec_round(b,d,a,c,e,key[4],key[5],key[6],key[7],key[0],key[1],key[2],4);
    dec_round(a,b,c,d,e,key[5],key[6],key[7],key[0],key[1],key[2],key[3],3);
    dec_round(c,a,d,b,e,key[6],key[7],key[0],key[1],key[2],key[3],key[4],2);
    dec_round(d,c,b,a,e,key[7],key[0],key[1],key[2],key[3],key[4],key[5],1);
    dec_round(b,d,a,c,e,key[0],key[1],key[2],key[3],key[4],key[5],key[6],0);
...
static int __init belt_init(void)
{
    return crypto_register_alg(&belt_alg);
}
static void __exit belt_fini(void)
{
    crypto_unregister_alg(&belt_alg);
}
